...
st.markdown("""
    <style>
    /* 1. Full-screen width for mobile */
    .block-container {
    ...
    /* 2. The Submit Button inside the Mileage Form */
    ...
    /* 4. The Refresh Button at the bottom */
    div.stButton > button {
        height: 3.5em !important;
        width: 100% !important;
        font-weight: bold !important;
        ...
        for _, row in book_df.iterrows():
            if pd.notnull(row[name_key]) and pd.notnull(row[start_key]):
                try:
                    ...
                    # FIX: Add 1 day to make the end date inclusive on the calendar
                    ed_inclusive = ed + pd.Timedelta(days=1)
                    ...
                    calendar_events.append({
                        ...
                        "end": ed_inclusive.strftime('%Y-%m-%d'),
